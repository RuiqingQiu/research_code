...
double computeLeKernelRGB(double ***A, double ***B, int nFullyConnectedLayer)
{
	...
	const int rf = 5;
	double k, u, v;
	for (int i=0; i<n0; i++){for (int j=0; j<n0; j++){double *a = A[i][j];double *b = B[i][j];k0[i][j] = a[0]*b[0] + a[1]*b[1] + a[2]*b[2]; u0[i][j] = a[0]*a[0] + a[1]*a[1] + a[2]*a[2];v0[i][j] = b[0]*b[0] + b[1]*b[1] + b[2]*b[2];}}for (int i=0; i<n1; i++){for (int j=0; j<n1; j++){double kk=0, uu=0, vv=0;for (int x=0; x<rf; x++)for (int y=0; y<rf; y++){kk += k0[i+x][j+y];uu += u0[i+x][j+y];vv += v0[i+x][j+y];}k1[i][j] = arcCosKernel(kk,sqrt(uu*vv));u1[i][j] = uu;v1[i][j] = vv;}}

for (int i=0; i<n2; i++){for (int j=0; j<n2; j++){int x = 2*i;int y = 2*j;k2[i][j] = k1[x][y] + k1[x+1][y] + k1[x][y+1] + k1[x+1][y+1];u2[i][j] = u1[x][y] + u1[x+1][y] + u1[x][y+1] + u1[x+1][y+1];v2[i][j] = v1[x][y] + v1[x+1][y] + v1[x][y+1] + v1[x+1][y+1];}}
...
for (int i=0; i<n4; i++){for (int j=0; j<n4; j++){int x = 2*i;int y = 2*j;k4[i][j] = k3[x][y] + k3[x+1][y] + k3[x][y+1] + k3[x+1][y+1];u4[i][j] = u3[x][y] + u3[x+1][y] + u3[x][y+1] + u3[x+1][y+1];v4[i][j] = v3[x][y] + v3[x+1][y] + v3[x][y+1] + v3[x+1][y+1];}}
