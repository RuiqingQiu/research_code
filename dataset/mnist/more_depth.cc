...
for (int i=0; i<n2; i++){for (int j=0; j<n2; j++){double kk=0, uu=0, vv=0;for (int x=0; x<rf; x++)for (int y=0; y<rf; y++){kk += k1[i+x][j+y];uu += u1[i+x][j+y];vv += v1[i+x][j+y];}k2[i][j] = arcCosKernel(kk,sqrt(uu*vv));u2[i][j] = uu;v2[i][j] = vv;}}
	 ...
	 //Subsampling by 2
for (int i=0; i<n3; i++){for (int j=0; j<n3; j++){int x = 2*i;int y = 2*j;k3[i][j] = k2[x][y] + k2[x+1][y] + k2[x][y+1] + k2[x+1][y+1];u3[i][j] = u2[x][y] + u2[x+1][y] + u2[x][y+1] + u2[x+1][y+1];v3[i][j] = v2[x][y] + v2[x+1][y] + v2[x][y+1] + v2[x+1][y+1];}}
...
for (int i=0; i<n5; i++){for (int j=0; j<n5; j++){double kk=0, uu=0, vv=0;for (int x=0; x<rf; x++)for (int y=0; y<rf; y++){kk += k4[i+x][j+y];uu += u4[i+x][j+y];vv += v4[i+x][j+y];}k5[i][j] = arcCosKernel(kk,sqrt(uu*vv));u5[i][j] = uu;v5[i][j] = vv;}}
	 ...
	 //Subsampling by 2
for (int i=0; i<n6; i++){for (int j=0; j<n6; j++){int x = 2*i;int y = 2*j;k6[i][j] = k5[x][y] + k5[x+1][y] + k5[x][y+1] + k5[x+1][y+1];u6[i][j] = u5[x][y] + u5[x+1][y] + u5[x][y+1] + u5[x+1][y+1];v6[i][j] = v5[x][y] + v5[x+1][y] + v5[x][y+1] + v5[x+1][y+1];}}
