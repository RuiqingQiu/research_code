...
double computeLeKernel(double **A, double **B, int nFullyConnectedLayer)
{
	double kAB, normAB;
	const int n0 = 32;
	double k0[n0][n0];
	...
	const int rf = 3;
	double k, u, v;
    for (int i=0; i<n0; i++){for (int j=0; j<n0; j++){
        double a = A[i][j];
        double b = B[i][j];
        k0[i][j] = a*b;
        u0[i][j] = a*a;
    ...
    //Convolutional Layer
    for (int i=0; i<n7; i++){for (int j=0; j<n7; j++){double kk=0, uu=0, vv=0;for (int x=0; x<rf; x++)for (int y=0; y<rf; y++){kk += k6[i+x][j+y];uu += u6[i+x][j+y];vv += v6[i+x][j+y];}k7[i][j] = arcCosKernel(kk,sqrt(uu*vv));u7[i][j] = uu;v7[i][j] = vv;}}

    //Convolutional Layer
    for (int i=0; i<n8; i++){for (int j=0; j<n8; j++){double kk=0, uu=0, vv=0;for (int x=0; x<rf; x++)for (int y=0; y<rf; y++){kk += k7[i+x][j+y];uu += u7[i+x][j+y];vv += v7[i+x][j+y];}k8[i][j] = arcCosKernel(kk,sqrt(uu*vv));u8[i][j] = uu;v8[i][j] = vv;}}

    //Subsampling by 2
    for (int i=0; i<n9; i++){for (int j=0; j<n9; j++){int x = 2*i;int y = 2*j;k9[i][j] = k8[x][y] + k8[x+1][y] + k8[x][y+1] + k8[x+1][y+1];u9[i][j] = u8[x][y] + u8[x+1][y] + u8[x][y+1] + u8[x+1][y+1];v9[i][j] = v8[x][y] + v8[x+1][y] + v8[x][y+1] + v8[x+1][y+1];}}

    //Fully Connected Layer
    k = u = v = 0;for (int i=0; i<n9; i++){for (int j=0; j<n9; j++){k += k9[i][j];u += u9[i][j];v += v9[i][j];}}	normAB = sqrt(u*v);
